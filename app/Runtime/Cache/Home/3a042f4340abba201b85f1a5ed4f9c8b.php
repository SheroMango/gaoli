...
<div class="tabs">
    <ul>
        <li class="current"><a href="javascript:void(0)" class="current">新闻信息</a></li>
    </ul>
</div>
<div class="edit">
    <form method="post" action="<?php echo U('Home/News/info');?>" enctype="multipart/form-data">
        <input type="hidden" name="id" value="<?php echo ($info["id"]); ?>" />
        <input type="hidden" name="pid" value="<?php echo ($pid); ?>">
        <dl>
            <dt>新闻分类：</dt>
            <dd><select name="type_id" value="<?php echo ($info["type_id"]); ?>">
                <!-- volist 作用：循环输出数组 -->
                <?php if(is_array($typeList)): $i = 0; $__LIST__ = $typeList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$type): $mod = ($i % 2 );++$i;?><option value="<?php echo ($type["id"]); ?>" <?php if(($info[type_id]) == $type[id]): ?>selected<?php endif; ?>><?php echo ($type["title"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
            </select></dd>
        </dl>
        <dl>
            <dt>新闻标题：</dt>
        ...
        <dl>
            <dt>模版：</dt>
            <dd><select name="tpl_id">
                <?php if(is_array($tplList)): $i = 0; $__LIST__ = $tplList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>" <?php if(($info[tpl_id]) == $vo[id]): ?>selected<?php endif; ?>><?php echo ($vo["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
            </select></dd>
        <dl>
            <dt>排序：</dt>
