...
<div class="content">
  ...
  <div class="tabs">
    <ul>
      <li class="current"><a href="javascript:void(0)" class="current">新闻列表</a></li>
    </ul>
  </div>
  <!--END 选项卡-->
 <!--搜索 BEGIN-->
<div class="search">
    <form action="<?php echo U('Home/News/ls');?>" method="post">
        <table width="100%">
            <td width="100">
...
<!--END 搜索--> 
  ...
  <!--工具栏 BEGIN-->
  <div class="toolbar">
      <a href="<?php echo U('Home/News/addNews');?>" class="btn"><span>添加新闻</span></a>
  </div>
  <!--END工具栏 BEGIN-->
  
  <!--列表 BEGIN-->
  <div class="list">
      <form action="<?php echo U('Home/News/del');?>" method="post" class="del-form">
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <thead>
          <tr>
            <th width="5%"><input type="checkbox" class="check-all" /></th>
            <th width="20%"><a href="<?php echo U('Home/News/ls',array('sort'=>'title','type'=>$type));?>">标题</th>
            <th width="20%">描述</th>
            <th width="20%"><a href="<?php echo U('Home/News/ls',array('sort'=>'sort','type'=>$type));?>">排序</th>
            <th width="20%"><a href="<?php echo U('Home/News/ls',array('sort'=>'time_modify','type'=>$type));?>">更新时间</th>
            <th width="15%">操作</th>
          </tr>
        </thead>
        <tbody>
          <?php if(empty($list)): ?><tr>
              <td colspan="11" class="empty">暂无相关新闻</td>
            </tr>
          <?php else: ?>
          <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                ...
                <td>
                    <a href="<?php echo U('Home/News/updateNews',array('id'=>$vo[id]));?>" class="mr10">编辑</a>
                    <a href="<?php echo U('Home/News/del',array('id'=>$vo[id]));?>">删除</a>
                </td>
